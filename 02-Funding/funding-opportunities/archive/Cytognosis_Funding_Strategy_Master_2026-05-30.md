...
```mermaid
gantt
    title Cytognosis funding waves — Jun 2026 to 2027
    dateFormat YYYY-MM-DD
    axisFormat %b
    section Wave 0 (now)
    AWS Imagine (Jun 5)              :crit, 2026-06-01, 5d
    NIMH U01 sensor (Jun 15)         :crit, 2026-06-01, 15d
    ARPA-H IGoR (Jun 25)             :crit, 2026-06-01, 25d
    NSF X-Labs Phase 0 (Jul 13)      :crit, 2026-06-01, 43d
    HBNet / FRO / credits (rolling)  :active, 2026-05-30, 10d
    section Wave 1 (1-3 mo)
    ARPA-H PHO + HSF ISOs            :2026-06-15, 60d
    NIMH R21/R24/R34                 :2026-06-15, 75d
    McGovern · Emergent · NVIDIA     :2026-06-10, 60d
    section Wave 2 (3-6 mo)
    Fast Forward 2027 (Jul 30-Sep 8) :2026-07-30, 40d
    CZI EOSS LOI (Oct 18)            :2026-09-15, 33d
    Brains/Activate/Astera (~Oct)    :2026-10-01, 30d
    MH cycles (BBRF/Tiny Blue/AFSP)  :2026-08-01, 90d
    section Wave 3 (6-18 mo / PBC)
    Convergent FRO full process      :2026-07-01, 360d
    Wellcome Leap / MH Award         :2026-09-01, 240d
    EU Horizon 2027 · SBIR (PBC)     :2027-02-01, 120d
```
...
```mermaid
flowchart LR
    S1["**STAGE 1 — DOCS (this delivery)**<br/>• Funnel + scoring (65 opps)<br/>• Master doc + workbook<br/>• PAC companion<br/>• Meta-source layer<br/>• Stage-2 spec (below)"]
    REV["Review &<br/>sign-off"]
    S2["**STAGE 2 — AUTOMATION + MONDAY**<br/>• Scraper over free feeds<br/>• Normalize → dedupe → score<br/>• Write to Monday v1.2 board<br/>• In-Monday update tooling<br/>• Weekly (Monday) cadence"]
    S1 --> REV --> S2
    style S1 fill:#3B7DD6,color:#fff
    style REV fill:#E0309E,color:#fff
    style S2 fill:#8B3FC7,color:#fff
```
...
```mermaid
flowchart TB
    subgraph SRC[Sources · free feeds]
        A1[Grants.gov Search2 API]
        A2[SAM.gov Opportunities v2]
        A3[NIH Guide RSS]
        A4[NSF RSS]
        A5[Federal Register API]
        A6[EU SEDIA CSV]
    end
    SRC --> N[Normalizer<br/>→ canonical Opportunity record]
    N --> D[Deduper<br/>fuzzy match vs Monday board + prior runs]
    D --> SC[Scorer<br/>FitScore rubric + keyword/eligibility heuristics]
    SC --> F{New or<br/>materially changed?}
    F -- yes --> M[Monday GraphQL<br/>create/update item + subitems]
    F -- no --> LOG[(run log / no-op)]
    M --> REV[Weekly digest<br/>+ board 'Needs triage' group]
    style SRC fill:#EEF2FB
