# Y Combinator Application — Refreshed Draft

> **Status:** Prep, deadline Jul 27 · **Date:** 2026-07-16 · **Author:** Shahin Mohammadi

## Refresh note (read first)

This is a refresh of `Y Combinator Application (Summer 2026).md`, which was drafted for the
Summer 2026 batch but never submitted (the folder's INDEX had it marked "Parked"). Web
verification (2026-07-16) confirms the next real deadline is the **Y Combinator Fall 2026
batch, on-time deadline Jul 27, 2026, 8pm PT** (decisions by Aug 28, 2026; batch runs
October to December 2026 in San Francisco). Apply at ycombinator.com/apply.

Only the batch-specific fields below were changed (batch name, deadline framing, and the
"same idea as previous batch" answer). The rest of the content, company description, founder
story, competitive landscape, monetization, and the PBC/Yar for-profit framing, is carried
over unchanged from the Summer 2026 draft, because it is not batch-dependent. **Before
submitting, confirm with Shahin whether any of the progress claims below (TestFlight beta
status, user count, cofounder search) have changed since that draft was written; do not
submit stale progress numbers as current.**

**Recommendation: apply for Fall 2026. Do not wait for a later batch.** Rationale:

- YC's own criteria reward founder-market fit and a working MVP, not completed traction; the
  existing draft already frames "engine runs, no users yet" honestly and that is an accepted
  stage for a consumer AI application, not a disqualifier.
- The two prior rejections (Winter 2025, Spring 2026) pitched the nonprofit science platform,
  a poor fit for YC's model. The Yar/for-profit PBC pivot documented in this draft addresses
  that root cause; it has not yet been tested with YC.
- Applying costs nothing but time, and the batch itself is the forcing function this draft
  already argues for (cofounder search, first users, retention data by Demo Day).
- Given the Oct 1 runway cliff on the nonprofit side, a YC outcome (or even the application
  process discipline) helps the highest-upside personal-runway path without displacing the
  grant-funded nonprofit track, which continues in parallel.
- Counter-consideration, flagged not resolved: if Yar has made no forward progress at all
  since the Summer 2026 draft (no TestFlight, no design partners engaged), the honest answers
  to "are people using your product" and "how far along are you" would be identical to a
  batch YC already effectively passed on by not being submitted. Applying with unchanged
  facts is still reasonable (YC does not penalize a slightly later batch), but if a fresher
  proof point (even one design partner using it) can be secured before Jul 27, the odds
  improve. This is worth a few days of Shahin's attention before submission, not a reason to
  skip the batch.
...
Working MVP (TestFlight build in preparation; CONFIRM current status with Shahin before submitting).

### What is your company going to make?

Yar is a private, on-device AI companion for the roughly one in five adults whose brains work differently: people with ADHD, autistic adults, and twice-exceptional and late-diagnosed adults.
...
CONFIRM WITH SHAHIN before submitting: is this still accurate, or has TestFlight shipped since the Summer 2026 draft? As last drafted: I have a working MVP. On-device Gemma 4 turns voice and text into validated, typed knowledge-graph objects, stored locally, with optional confirmed sync to a personal knowledge graph. Product Milestone 1, the end-to-end mobile voice loop (capture, on-device Gemma, Cytoplex-Lite safety gate, confirmed knowledge-graph write), is shipped. The build has 243 passing automated tests, including 93 tests on the safety layer alone, and zero lint violations across the codebase. What I do not have yet: real users. The honest state is "the engine runs; now I put it in people's hands."
...
Flutter (mobile), FastAPI (backend), on-device Gemma 4 via LiteRT and Ollama for local inference, SQLite for local storage, optional Anytype sync for the personal knowledge graph, and our own Cytoplex safety protocol in front of every model call. Schemas are LinkML-style YAML so every AI output is validated before it is written. Cursor for development. CONFIRM WITH SHAHIN: the storage and sync stack (SQLite vs. a graph database, and the cross-device sync protocol) was still under active evaluation as of mid-June 2026; do not overstate this as settled. The Tier 3 wellbeing layer will add a paralinguistic voice pipeline (openSMILE, HuBERT) down the road.

### Are people using your product?

CONFIRM WITH SHAHIN before submitting; as last drafted: Not in production yet, but not from a standing start either. I am embedded in tightly-knit neurodivergent and queer/trans communities that are exactly Yar's first users, and they are already pulling: friends have sent me their own whole-genome data unprompted, and several, including engineers who build and wear their own sensors, are ready to be design partners and first users today. The next 90 days turn this warm cohort into a measured, retained beta.

### When will you have a version people can use?

A TestFlight beta within 4 to 6 weeks; a public App Store launch within the batch. The capture-and-structure core is already functional, so the gap to "real people using it daily" is packaging and onboarding, not core technology. CONFIRM WITH SHAHIN: recompute this timeline from today's date if the Summer 2026 timeline has already elapsed without a shipped beta.
...
Yes. I applied twice pitching the full Cytognosis platform as a nonprofit: Winter 2025 and Spring 2026, both "GPS for Health" pitches with AI models and biosensors. It is real and I am still building it, but it is a 10-year, capital-intensive science effort, a poor fit for a 3-month YC sprint and an awkward fit for the nonprofit track. I drafted a Yar-as-for-profit pivot for the Summer 2026 batch but did not submit it while I finished the MVP build. This Fall 2026 application is that same pivot, refreshed: the same mission's consumer wedge, shippable now, applying as a for-profit for the first time rather than a nonprofit. I stopped pitching the infrastructure and started shipping the product.

### If you have already participated in an incubator/accelerator?

I completed NUCLEATE (2019). I have grant applications pending (ARPA-H, Coefficient Giving, EA Funds) that fund the upstream nonprofit science platform, separate from Yar the product. A Fast Forward Academy application (a nonprofit-only program, not applicable to Yar's for-profit entity) is separately in prep for the Cytognosis Foundation side.
...
Fall 2026 (on-time deadline Jul 27, 2026, 8pm PT; decisions by Aug 28, 2026; batch runs October to December 2026 in San Francisco).
