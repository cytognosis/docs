...
Compiled 2026-06-14 | Profile: **internal** (includes restricted sections, do not distribute)
...
- [Our contribution: TA1 Pillar 2b factorized-PRS (PROPRIETARY)](sections/31_contrib_TA1_factorized_PRS_PROPRIETARY.md) `restricted`
- [Our contribution: SPEAR perturbation model (UNDER REVIEW)](sections/32_contrib_SPEAR_PROPRIETARY.md) `restricted`
...
- [Phase I anchor experiment (RESTRICTED: personal genomic data)](sections/41_phase1_anchor_experiment_RESTRICTED.md) `restricted`
...
- [Proposal evolution: variants as key contributions](sections/70_proposal_evolution_crosswalk.md) `restricted`
- [Open decisions, gaps, and pre-submission actions](sections/90_open_decisions_and_gaps.md) `restricted`
...
**Cellanome R3200** provides a programmable CellCage platform integrating live-cell imaging with same-cell pooled-CRISPR sequencing (Perturb-LINK), enabling simultaneous morphological and transcriptomic readouts in neuronal models. Cellanome is the **industry experimental arm of TA4** (advancing as of 2026-06-17; operating model under discussion: R3200 placed in a PsychIGoR lab vs. send-cells-out).

**Element Biosciences AVITI24 / Teton CytoProfiling** is the multi-modal same-cell platform used by the **academic experimental arm (Matt Tegtmeyer lab, Purdue)**. It delivers co-detection of RNA (350-plex subcellular), protein and phospho-protein (50-plex), Cell-Painting-style organelle morphology (nucleus, membrane, actin, ER, Golgi, mitochondria; onboard segmentation + feature extraction), and in-situ CRISPR-guide sequencing (DISS) from fixed cells at subcellular resolution across hundreds of thousands to millions of cells; next-day results; has a dedicated neuroscience panel (neurodifferentiation, neurotransmission/synaptic, neurodegeneration, neuroinflammation). Element = fixed-cell in-situ at massive scale; Cellanome = live-cell temporal with nanoscale single-cell isolation. The two platforms are **orthogonal/complementary**; running the same variant lines on both provides a built-in cross-arm concordance check aligned with the program's 85% concordance gates. Element's Cell-Painting-like morphology output feeds directly into Anne Carpenter's computational morphology models.
...
TA4 has two experimental arms. The **academic arm** is the **Matt Tegtmeyer lab (Purdue)**, running all wet-lab experiments on iPSC-neuron disease models with the **Element AVITI24 / Teton CytoProfiling** platform (RNA 350-plex + protein/phospho 50-plex + Cell-Painting-style morphology + CRISPR-guide DISS; neuroscience panel). The **industry arm** is **Cellanome** (R3200 + Perturb-LINK; live-cell temporal readouts). **Anne Carpenter** provides the **computational morphology/imaging-model layer** (interpretable models; no wet-lab bench); she consumes readouts from both arms and bridges into TA1/TA2. **Illumina** contributes the Billion Cell Atlas perturbation resource as in-kind resource sharing (Lab 3). Each lab publishes machine-readable capability manifests, validates against shared IV&V calibration artifacts, and demonstrates cross-laboratory concordance (Phase I target: 85%; Phase II and III: 90%). The two-arm design provides a built-in cross-arm concordance check: the same variant lines run on Element AVITI24 (fixed-cell in-situ, massive scale) and Cellanome R3200 (live-cell temporal), with Anne's computational models as the common analysis layer.
...
| **Matt Tegtmeyer lab (Purdue)** | Academia | TA4 academic experimental arm (all wet-lab experiments; iPSC-neuron disease models; Element AVITI24 / Teton CytoProfiling) | Matthew Tegtmeyer, PhD (Purdue faculty) | Confirmed 2026-06-15 |
| **Anne Carpenter (computational)** | Academia (Broad Institute; Purdue/IPAI ~Sep 2026) | TA4 computational morphology/imaging-model lead; interpretable models; TA4 analysis + TA1/TA2 bridge; **no wet-lab bench** | Anne Carpenter, PhD (Senior Director, Broad Imaging Platform) | Warm; dialogue active (~90%) |
| **Cellanome** | For-profit | TA4 industry experimental arm (live-cell + same-cell Perturb-seq; R3200 + Perturb-LINK) | Dwight Baker, SVP Product Development | Advancing (2026-06-17 call); operating model pending pricing |
...
| TA4 academic experimental arm | Matthew Tegtmeyer | confirmed (Purdue faculty; confirmed 2026-06-15) |
| TA4 computational morphology/imaging-model lead (no bench) | Anne Carpenter | confirmed (~90%; joint Purdue/Broad; purely computational) |
| Project Manager | Patricia Purcell | verbally agreed (terms TBD) |
| Software and Systems Architect | Anna Merkoulovitch (lead candidate) | in discussion / recruiting |
...
| TA4 laboratories (two or more) | Carpenter/Broad, Cellanome, Illumina, SPOC; Panome (alternate) | in discussion |
...
| **Cellanome (TA4)** | Advancing; NDA pending; calcium-imaging/firing scope being confirmed | Close NDA and confirm scope |
| **Illumina (TA4)** | "Key contact in discussion"; soften or confirm language before submission | Confirm commitment or reword |
| **Transfyr (TA3 candidate)** | Held pending COI check (founder is a former ARPA-H director) | Clear COI or keep excluded |
