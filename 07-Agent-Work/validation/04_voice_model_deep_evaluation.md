...
Based on the three-capability evaluation, the recommended architecture for Yar is:
...
flowchart TB
    ...
    BaseModel -->|"transcripts + selected\nbiomarker summaries\n(not raw audio)"| SuperModel
    Guidance -->|"update_guidance()\ninject_priority_message()"| BaseModel
    ...
    style Parallel fill:#1E1E32,color:#E0E0ED
    style Sensor fill:#E0309E,color:#fff
    style BaseModel fill:#3B7DD6,color:#fff
    style LocalStore fill:#14A3A3,color:#fff
```

### 5.1 Phase 1 (Now): Gemma 4 Cascade + Parallel Sensor

- Base model: Gemma 4 E4B via LiteRT-LM (audio in, text out)
- TTS: Platform native (upgrade to Kokoro later)
- Sensor: openSMILE eGeMAPSv02 running in parallel on audio stream
- Supervisor: Gemma 4 26B MoE via Ollama, guidance via function calls
- Storage: VocalBiomarkerFrame in local SQLite

### 5.2 Phase 2 (6-12 months): Add HuBERT SER + Longitudinal Tracking

- Add HuBERT-large fine-tuned SER for categorical + dimensional emotion
- Add SessionVocalProfile aggregation
- Add ADHD/ASD marker derivation (requires user baseline establishment)
- Upgrade TTS to Kokoro 82M
- Evaluate Cactus for hybrid routing

### 5.3 Phase 3 (12-18 months): Evaluate Moshi Migration

- If user research shows full-duplex naturalism matters for Yar (knowledge capture + companion, not just therapy)
- MoshiRAG provides native knowledge grounding for companion conversations
- Supervisor channel becomes architecturally native instead of engineered
- Evaluate MoshiVis for multimodal (camera + voice) sessions

---

## 6. Model Selection Decision Matrix (Updated)

| If your priority is... | Edge Model | Supervisor | Why |
|---|---|---|---|
| **Fastest to Yar v1** | Gemma 4 E4B (cascade) | Gemma 4 26B MoE | Function calling, Apache 2.0, 35+ langs |
| **Best supervisor control** | Moshi 7B (full-duplex) | Qwen3.5-Omni (audio reasoning) | Native interrupt + RAG; best nonverbal reasoning |
| **Best nonverbal understanding** | Any + **HuBERT/openSMILE** | Qwen3.5-Omni | Quantified features from sensor; Qwen reasons about them |
| **Best longitudinal tracking** | Any + **HuBERT/openSMILE** | Gemma 4 26B MoE | Sensor produces quantified data; any model combo works |
| **Smallest edge footprint** | LFM2.5-Audio-1.5B | Gemma 4 26B MoE | Sub-1GB; but weak on nonverbal, needs heavy sensor |
| **Full open-source (Apache 2.0)** | Gemma 4 E4B + Kokoro | Gemma 4 26B MoE | Entire stack Apache 2.0 |
