...
Yar is a **local-first cognitive companion for neurodivergent knowledge capture**. It captures text, voice transcripts, webpages, and messages; routes through local Gemma via Ollama; structures them into typed graph objects; applies CAP-Lite guardrails; stores in local SQLite (with optional Anytype MCP write).
...
| SQLite as default storage | Local-first; no server setup; neurodivergent users shouldn't configure databases |
| Anytype as optional external write target | Users choose their own graph system; Yar works without it |
| Gemma on-device via LiteRT | Privacy (no cloud); neurodivergent users need local control |
